...
func providerConnectivityCheck(nsxClient *nsxt.APIClient) error {
	...
	_, httpResponse, err := nsxClient.GroupingObjectsNsGroupsApi.GetMemberTypes(nsxClient.Context, "Dummy", nil)
	if err != nil {
...
func ConnectNSXManager(host string, un string, pw string) *nsxt.APIClient {
	...
	fmt.Println("Creating NSX Client")
	...
	fmt.Println("Client Created: Checking Connectivity")
...
func policyConnectivityCheck(nsxClient *nsxt.APIClient) error {
	// Connectivity check - get a random object to check connectivity and credentials
	// TODO(asarfaty): Use a list command which returns the full body, when the go vendor has one.
	_, httpResponse, err := nsxClient.PolicyInventoryGroupsApi.ReadGroupForDomain(nsxClient.Context, "default", "Infoblox")
	if err != nil {
		if httpResponse == nil || (httpResponse.StatusCode == 401 || httpResponse.StatusCode == 403) {
			return fmt.Errorf("NSXT provider connectivity check failed: %s", err)
		}
...
func ConnectNSXPolicyManager(host string, un string, pw string) *nsxt.APIClient {
	...
	fmt.Println("Client Created: Checking Connectivity")
	if policyConnectivityCheck(nsxClient) != nil {
		fmt.Println(err)
	}
	fmt.Println("Connection to NSX-T Manager", host, "Successful")
